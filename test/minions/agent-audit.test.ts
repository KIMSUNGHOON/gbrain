...
afterEach(() => {
  if (prevAuditDir === undefined) {
    delete process.env.GBRAIN_AUDIT_DIR;
  } else {
    process.env.GBRAIN_AUDIT_DIR = prevAuditDir;
  }
...
describe('agent-audit (v0.38 Slice 3 — D4 JSONL trail for submit_agent)', () => {
  ...
  describe('logAgentSubmission()', () => {
    it('writes a JSONL line with all expected fields', () => {
      logAgentSubmission({
    ...
    it('appends multiple events to the same weekly file', () => {
      for (let i = 0; i < 3; i++) {
        logAgentSubmission({
          client_id: 'c',
          job_id: i,
    ...
    it('NEVER logs prompt content (only byte count)', () => {
  ...
  describe('readRecentAgentEvents()', () => {
    it('returns events written within the window, newest first', () => {
      logAgentSubmission({
        client_id: 'a',
        job_id: 1,
        model: 'm',
    ...
    it('returns empty when audit dir doesnt exist yet', () => {
