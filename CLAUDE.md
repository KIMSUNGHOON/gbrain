...
calibration on top. Migration renumbered v74→v79 during master merge —
v0.37.0.0's autonomous-remediation wave claimed v68-v78 in parallel. Pinned by
R1-R5 in `test/takes-resolution.test.ts` and `test/migrate.test.ts`'s
v79 structural + PGLite round-trip suite (CHECK admits unresolvable+NULL, still
rejects partial+true and unresolvable+true|false, pre-v79 NULL/NULL rows survive).
